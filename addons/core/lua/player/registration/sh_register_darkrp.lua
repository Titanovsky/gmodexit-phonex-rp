...
if ( AMB.config.gamemode == "DarkRP" ) then
    AmbRegister.cam_vector = Vector( -2662.816406, -1945.769043, 380.942566 ) -- rp_downtown_v4_exl
    AmbRegister.cam_angle = Angle( 29.035515, 26.733103, 0.000000 )
elseif ( AMB.config.gamemode == "Sandbox" ) then
...
if ( SERVER ) then
    ...
    net.Receive( "amb_players_done_connection", function( len, caller )
        ...
        if ( caller ~= ePly ) then caller:Kick('Hight Ping (>750)') return end
    ...
    net.Receive( "amb_players_done_registration_darkrp", function( len, caller )
        ...
        elseif ( AMB.config.gamemode == "DarkRP" ) then
            print('amb_players_done_registration_darkrp')
            AmbStats.Players.authorizationPlayer( ePly )
        end
    end )
...
elseif ( CLIENT ) then
    ...
    function AmbRegister.startRegRPFrame( frame, send )
        ...
        if ( frame == 1 ) then
            ...
            amb_panel_reg:SetDraggable( false )
            amb_panel_reg:ShowCloseButton( false )
        ...
        elseif ( frame == 2 ) then
            ...
            box_nationality.OnSelect = function( self, index, value )
                AmbRegister.self_nation = value
            end
    ...
    net.Receive( "amb_players_register_person", function( len )
        ...
        -- print('amb_players_register_person OUTPUT` [ DARKRP ]') -- debug
        -- print( flag ) -- debug
        ...
        elseif ( flag == 2 ) then
            print('amb_players_register_person [ DARKRP ]')
            AmbRegister.startRegistrationRolePlay( true )
        end
    end )


    hook.Add( "InitPostEntity", "amb0_x84", function()
        AmbRegister.start()
