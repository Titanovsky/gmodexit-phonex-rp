...
function AmbStats.Players.rpRegisterPlayer( ePly )
    ...
    elseif ( AMB.config.gamemode == "DarkRP" ) then
        local query = Format( "%s, %s, %f, %f, %f, %s, %s, %s, %f", s( ePly:SteamID() ), s( ePly:GetNWString('amb_players_name') ), start_money_darkrp, start_level, start_exp, s( ePly:GetNWString('amb_players_skin') ), s( ePly:GetNWString('amb_players_nation') ), s( ePly:GetNWString('amb_players_sex') ), ePly:GetNWFloat('amb_players_age') )
        AmbDB.insertDate( amb_db_players_darkrp, 'ID, Name, Money, Level, Exp, Skin, Nation, Sex, Age', query )
    end
end
...
function AmbStats.Players.initialPlayer( ePly )
    if ( AMB.config.gamemode == "Sandbox" ) then
        if ( AmbDB.selectDate( amb_db_players_stats, 'ID', 'ID', ePly:SteamID() ) == ePly:SteamID() ) and ePly.authorized == false or ePly.authorized == nil then
            ...
            -- local money = AmbDB.selectDate( amb_db_players_stats, 'Bio_Essensia', 'ID', ePly:SteamID() ) -- уже не работает
            ...
            ePly:SetNWFloat( 'amb_players_money', ePly:getDarkRPVar('money') ) 
...
function AmbStats.Players.setStats( ePly, stats, value )
    ...
    if ( stats == "$" or stats == "M" or stats == "money" or stats == "Money" or stats == 1 ) then
        ...
        ePly:setDarkRPVar( 'money', value )
        DarkRP.storeMoney( ePly, value )
    ...
    elseif ( stats == "&" or stats == "sex" or stats == "Sex" or stats == 9 ) then
        value = tostring( value )
        ePly:SetNWString( 'amb_players_sex', value )
        AmbLib.notifySend( ePly,'Вам поменяли пол: '..value, 4, 6, 'buttons/button10.wav' )
        AmbDB.updateDate( amb_db_players_darkrp, 'Sex', value, 'ID', id )

    elseif ( stats == "-" or stats == "nation" or stats == "Nation" or stats == 10 ) then
        value = tostring( value )
        ePly:SetNWString( 'amb_players_nation', value )
        AmbLib.notifySend( ePly,'Вам изменили нацию: '..value, 4, 6, 'buttons/button10.wav' )
        AmbDB.updateDate( amb_db_players_darkrp, 'Nation', value, 'ID', id )
    end
end
...
function AmbStats.Players.addStats( ePly, stats, value )
    ...
    if ( stats == "$" or stats == "M" or stats == "money" or stats == "Money" or stats == 1 ) then
        ...
        local wallet = ePly:getDarkRPVar( 'money' )
        AmbLib.notifySend( ePly, 'Долларчики: '..value, 0, 3, 'buttons/button9.wav' )
        value = wallet + value
    ...
    elseif ( stats == "!" or stats == "lvl" or stats == "Level" or stats == 2 ) then
        ...
        AmbLib.notifySend( ePly, 'Уровень: '..value, 0, 3, 'buttons/button9.wav' )
        value = level + value
...
function AmbStats.Players.getStats( ePly, stats )
    if ( stats == "$" or stats == "M" or stats == "money" or stats == "Money" or stats == 1 ) then
        return ePly:getDarkRPVar('money')
    elseif ( stats == "!" or stats == "lvl" or stats == "Level" or stats == 2 ) then
...
function AmbStats.Players.newLevel( ePly )
    if ( tonumber( ePly:GetNWFloat('amb_players_exp') ) >= tonumber( ePly:GetNWFloat('amb_players_max_exp') ) ) then
        ...
        AmbLib.notifySend( ePly, 'Новый Уровень!', 0, 8, 'buttons/bell1.wav' )
        AmbStats.Players.limitCalc( ePly )
    else
...
function AmbStats.Players.limitCalc( ePly ) -- todo: change for table !
    if ePly:GetUserGroup() == 'user' then
        ...
        ePly:SetNWInt( 'amb_players_limit_sents', tonumber( ePly:GetNWInt('amb_players_level') ) * 5 ) -- [ ENTITIES ]
        ...
        ePly:SetNWInt( 'amb_players_limit_npcs', 0 ) -- [ NPC ]
    ...
    elseif ePly:GetUserGroup() == 'vip' then
        ...
        ePly:SetNWInt( 'amb_players_limit_sents', tonumber( ePly:GetNWInt('amb_players_level') ) * 10 ) -- [ ENTITIES ]
        ...
        ePly:SetNWInt( 'amb_players_limit_npcs', tonumber( ePly:GetNWInt('amb_players_level') ) * 1 ) -- [ NPC ]
        ...
        ePly:SetNWInt( 'amb_players_limit_sents', tonumber( ePly:GetNWInt('amb_players_level') ) * 25 ) -- [ ENTITIES ]
        ...
        ePly:SetNWInt( 'amb_players_limit_npcs', tonumber( ePly:GetNWInt('amb_players_level') ) * 4 ) -- [ NPC ]
...
hook.Add( "PlayerInitialSpawn", "amb_0x2024", function( ePly )
    ...
    elseif ( AMB.config.gamemode == "DarkRP" ) then
        if sql.TableExists( amb_db_players_darkrp ) then
            if ( AmbDB.selectDate( amb_db_players_darkrp, 'ID', 'ID', ePly:SteamID() ) == ePly:SteamID() ) then
                AmbStats.Players.initialPlayer( ePly )
            end
        end
    end
