...
function AmbDuel.OpenRegister()
    ...
    combo_adversary.OnSelect = function( self, index, value )
        ...
        adversary = AmbDuel.FindName( value )
    ...
    send.DoClick = function( self )
        ...
        if AmbDuel.ValidationData() then

            return AmbDuel.CollectInfo()
...
function AmbDuel.FindName( sName )
...
function AmbDuel.ValidationData()

    if ( adversary == LocalPlayer() ) or ( adversary:IsPlayer() == false ) then return false end
    if ( award < AmbDuel.min_award ) or ( award > AmbDuel.max_award ) then return false end
