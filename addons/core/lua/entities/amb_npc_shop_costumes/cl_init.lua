...
local MAIN_TEXT_NPC			= "NPC Костюмы";
local MAX_DIST				= 600; -- максимальная дистанция отрисовки текста

local CannotDrawNPC			= true; -- включить оптимиз. фишку, чтобы прекращал отрисовку на расстоянии
local MAX_DIST_RENDER		= 1660000; -- макс. дистанция отрисовки модельки
...
local function AmbVGUI_drawPanelNPC(ent, dist) -- thx for _AMD_ (gm-donate крутой сервис)
	...
	if ( dist > MAX_DIST ) then
		...
		local x_name, _ = surface.GetTextSize( MAIN_TEXT_NPC )

		cam.Start3D2D( center + Vector( 0, 0, math.abs( max.z / 2 ) + 12 ), Angle( 0, rot, 90 ), 0.13 )
			draw.SimpleTextOutlined( MAIN_TEXT_NPC, MAIN_FONT, -x_name/2, 15, COLOR_TEXT, 0, 0, 1, Color(0,0,0) )
		cam.End3D2D()
