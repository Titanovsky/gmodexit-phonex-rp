...
function ENT:Draw()
    ...
    local rot = ( self:GetPos() - EyePos() ):Angle().yaw - 90 --  чтобы крутилась.
	...
	if ( self:GetPos():Distance( LocalPlayer():GetPos() ) < max_dist ) then
		...
		cam.Start3D2D( center + Vector( 0, 0, math.abs( max.z / 2 ) + 4 ), Angle( 0, rot, 90 ), 0.13 )
	        draw.SimpleTextOutlined( 'Покупатель Метамфетамина', font, 0, 0, AMB_COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, AMB_COLOR_BLUE )
	    cam.End3D2D()
	end
end
