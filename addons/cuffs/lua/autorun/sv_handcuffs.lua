...
CreateConVar( "cuffs_restrictarrest", 1, true )

hook.Add( "canArrest", "Cuffs RestrictArrest", function( cop, crim ) // DarkRP Arrest hook

	if IsValid(crim) and cvars.Bool("cuffs_restrictarrest") and not crim:IsHandcuffed() then return false, "Игрок должен быть в наручниках!" end

end)

hook.Add( "playerCanChangeTeam", "Cuffs RestrictTeam", function( ply, tm, force )

	if ply:IsHandcuffed() and cvars.Bool("cuffs_restrictteams") and not force then return false,"Нельзя поменять работу, когда вы в наручниках!" end
