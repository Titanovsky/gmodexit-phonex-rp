...
hook.Add( "CuffsCanHandcuff", "!Cuff ProtectAdmin", function( ply, target )
	...
	if IsValid(target) and target:IsPlayer() and target.adminmode then return false end
	...
	hook.Add( "HUDPaint", "Cuffs CuffedInteractPrompt", function()
		...
		if IsValid(cuff:GetFriendBreaking()) then
			...
			if cuff:GetFriendBreaking()==LocalPlayer() then
				...
				draw.SimpleText( "Освобождаем...", "HandcuffsText", w+1, TextPos+1, Col.TextShadow, TEXT_ALIGN_CENTER )

				draw.SimpleText( "Освобождаем...", "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER )
		...
		if cuff:GetRopeLength()>0 then
			...
			if IsValid(cuff:GetKidnapper()) then
				...
				if cuff:GetKidnapper()==LocalPlayer() then
					...
					local str = string.format( "%s перестать тащить за собой", (input.LookupBinding("+reload") or "[Reload]"):upper() )
				...
				local str = string.format( "%s тащить за собой", (input.LookupBinding("+reload") or "[Reload]"):upper() )
		...
		if cuff:GetCanBlind() then
			...
			local str = string.format( "%s %s глаза", (input.LookupBinding("+attack2") or "[PRIMARY FIRE]"):upper(), cuff:GetIsBlind() and "развязать" or "завязать" )
		...
		if cuff:GetCanGag() then
			...
			local str = string.format( "%s %s в рот кляп", (input.LookupBinding("+attack") or "[PRIMARY FIRE]"):upper(), cuff:GetIsGagged() and "вытащить" or "засунуть" )
