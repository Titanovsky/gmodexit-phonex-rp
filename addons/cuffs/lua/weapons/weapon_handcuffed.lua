...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		local str = string.format( "У вас руки сдержаны %s",

			((self:GetCuffStrength()>1.2) and " плотно") or ((self:GetCuffStrength()<0.8) and " свободно") or ""

		)

		if self:GetCuffRegen()>1.2 or self:GetCuffRegen()<0.8 then

			str = str.. string.format( " с %s материалом", ((self:GetCuffRegen()>1.2) and "крепким") or "непрочным" )
		...
		if self:GetIsGagged() then
			...
			draw.SimpleText( "Вам воткнули кляп в рот", "HandcuffsText", w+1, TextPos+1, Col.TextShadow, TEXT_ALIGN_CENTER )

			draw.SimpleText( "Вам воткнули кляп в рот", "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER )
		...
		if cvars.Bool( "cuffs_allowbreakout" ) then
			...
			str = string.format("%s освободиться", (input.LookupBinding("+attack") or "[Primary Fire]"):upper())
