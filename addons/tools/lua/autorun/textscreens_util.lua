...
if SERVER then
	...
	CreateConVar("sbox_maxtextscreens", "8", {FCVAR_NOTIFY, FCVAR_REPLICATED})
	...
	local function SpawnPermaTextscreens()
		...
		textscreens = file.Read("textscreens.txt", "DATA")
		...
		for k,v in pairs(ents.FindByClass("ts")) do
		...
		for k, v in pairs(textscreens) do
			...
			local textScreen = ents.Create("ts")
	...
	concommand.Add("SS_TextScreen", function(ply, cmd, args)
		...
		if not IsValid(ent) or ent:GetClass() ~= "ts" then return false end
		...
		if args[1] == "add" then
			...
			file.Write("textscreens.txt", util.TableToJSON(textscreens))
